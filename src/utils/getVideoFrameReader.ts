...
export async function getVideoFrameReader(videoFilepath: string, tmpDir: string, frameRate: number) {
  ...
  const filepaths = (await fs.promises.readdir(tmpDir)).map((file) => path.join(tmpDir, file)).sort();
