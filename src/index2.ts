...
const getVideo1Frame = await getVideoFrameReader(`${assetsPath}/pexels-4782135.mp4`, "tmp/video-1", frameRate);

console.log("Extracting frames from video 2...");
const getVideo2Frame = await getVideoFrameReader(`${assetsPath}/pexels-3576378.mp4`, "tmp/video-2", frameRate);

console.log("Extracting frames from video 3...");
const getVideo3Frame = await getVideoFrameReader(`${assetsPath}/pexels-2829177.mp4`, "tmp/video-3", frameRate);

const logo = await loadImage(`${assetsPath}/logo.svg`);

// Load fonts so we can use them for drawing
registerFont(`${assetsPath}/caveat-medium.ttf`, { family: "Caveat" });
registerFont(`${assetsPath}/chivo-regular.ttf`, { family: "Chivo" });
...
for (let i = 0; i < frameCount; i++) {
  ...
  renderMainComposition(context, image1, image2, image3, logo, canvas.width, canvas.height, time);
